...
Original-Verzeichnis `init\film\sql` bzw. `init\film\tls` in das Named Volume
...
das Named Volume `pg_init` kopiert. Danach wird das Verzeichnis `/tablespace/film`
...
DB `film` mit dem DB-User `film`anzulegen. Mit dem 2. Skript wird das Schema
`film` mit dem DB-User `film` als _Owner_ angelegt. Abschließend werden die
Tabellen angelegt und mit Testdaten aus den CSV-Dateien aus dem Verzeichnis
`/init/film/csv` im Named Volume `pg_init` gefüllt.

```shell
    docker compose exec db bash
        psql --dbname=postgres --username=postgres --file=/init/film/sql/create-db.sql
        psql --dbname=film --username=film --file=/init/film/sql/create-schema.sql

        psql --dbname=film --username=film --file=/init/film/sql/create-table.sql
        psql --dbname=film --username=postgres --file=/init/film/sql/copy-csv.sql
        exit
    docker compose down
```
...
Mit der Erweiterung _PostgreSQL_ für VS Code kann man die Datenbank `film` und
...
Im Untermenü _Databases_ von der Verbindung sieht man dann z.B. die Datenbank `film`
mit dem gleichnamigen Schema `film` und die Datenbank `postgres`.
Ebenso kann man man unter _Roles_ den DB-User `film` und den Superuser `postgres`
...
- Jetzt fehlt aber der Filmtitel.
- Wie kann ich diese Daten als CSV exportieren?

**BEACHTE**:

- _Bücher_ mit deutschem Umlaut als Plural der DB-Tabelle `film`.
