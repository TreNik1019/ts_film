...
Zunächst wird das Named Volume `swe_kc_data` für die künftigen Daten und das Named
Volume `swe_kc_tls` für das Zertifikat und den privaten Schlüssel angelegt:

```shell
    docker volume create swe_kc_data
    docker volume create swe_kc_tls
```
...
```shell
    ...
    docker run -v swe_kc_tls:/opt/keycloak/tls -v ./tls:/tmp/tls:ro `
...
Um das Zertifikat und den privaten Schlüssel in das Named Volume `swe_kc_tls` kopieren
...
`swe_kc_tls` kopiert. Danach wird der Linux-Owner und die -Gruppe jeweils auf `nonroot`
gesetzt.
...
und das Verzeichnis `/opt/keycloak/data` liegt wiederum im _Named Volume_ `swe_kc_data`.
...
```shell
    docker run --rm -it -v swe_kc_data:/opt/keycloak/data:ro oscarfonts/h2:2.3.232 bash
